...
# 문제) replace는 원본 문자열을 바꿀까요? 새로운 문자열을 만들까요? -> 새로운 문자열을 만든다
print(s.replace("Python", "C"))  # 전체 치환
print(s.replace("Python", "C", 1))  # 1개만 치환

# 판별 문자열 메소드 (isXXX())
print("123".isdigit())  # 숫자이면 True (음수(-),소숫점(.)은 숫자로 인식X)
print("123".isnumeric())  # 숫자이면 True (음수(-),소숫점(.)은 숫자로 인식X) 유니코드 숫자들도 인식
